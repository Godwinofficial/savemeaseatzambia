...
const AddWedding = () => {
    ...
    const getAutoGeneratedBio = (personType, name) => {
        const cleanName = (name || '').trim();
        const displayName = cleanName || (personType === 'bride' ? 'The bride' : 'The groom');

        if (personType === 'bride') {
            return `${displayName} is a kind, graceful, and loving woman who brings warmth, joy, and heart to every moment. She is admired for her beauty, strength, and the love she shares so effortlessly.`;
        }

        return `${displayName} is a thoughtful, loyal, and loving man who brings laughter, strength, and heart to every moment. He is admired for his kindness, humor, and the way he makes life feel full of love.`;
    };
    ...
    const handlePersonNameChange = (e) => {
        ...
        setFormData(prev => {
            const previousGeneratedBio = getAutoGeneratedBio(name === 'bride_name' ? 'bride' : 'groom', prev[name] || '');
            const nextGeneratedBio = getAutoGeneratedBio(name === 'bride_name' ? 'bride' : 'groom', value);
            const currentBio = prev[bioKey] || '';
            const shouldAutoFill = !currentBio || currentBio === previousGeneratedBio;

            return {
                ...prev,
                [name]: value,
                [bioKey]: shouldAutoFill ? nextGeneratedBio : currentBio
            };
        });
    };

    const fillGeneratedBio = (personType) => {
        const isBride = personType === 'bride';
        const nameKey = isBride ? 'bride_name' : 'groom_name';
        const bioKey = isBride ? 'bride_description' : 'groom_description';
        const name = formData[nameKey]?.trim();

        if (!name) return;

        setFormData(prev => ({
            ...prev,
            [bioKey]: getAutoGeneratedBio(personType, name)
        }));
    };

    useEffect(() => {
        const brideName = formData.bride_name?.trim();
        const groomName = formData.groom_name?.trim();

        if (brideName && (!formData.bride_description || formData.bride_description === getAutoGeneratedBio('bride', ''))) {
            setFormData(prev => ({
                ...prev,
                bride_description: getAutoGeneratedBio('bride', brideName)
            }));
        }

        if (groomName && (!formData.groom_description || formData.groom_description === getAutoGeneratedBio('groom', ''))) {
            setFormData(prev => ({
                ...prev,
                groom_description: getAutoGeneratedBio('groom', groomName)
            }));
        }
            ...
            <div className="couple-grid">
                <div className="person-card groom">
                    ...
                    <div className="form-group">
                        ...
                        <div className="input-with-icon">
                            ...
                            <input className="form-input" name="groom_name" value={formData.groom_name} onChange={handlePersonNameChange} placeholder="Enter groom's full name" />
                        </div>
                    </div>
                    <ImageUpload
                        label="Groom's Photo"
                    ...
                    <div className="form-group">
                        ...
                        <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                            <textarea className="form-textarea" name="groom_description" value={formData.groom_description} onChange={handleChange} placeholder="Share his story, personality, and interests..." style={{ flex: 1 }} />
                            <button
                                type="button"
                                onClick={() => fillGeneratedBio('groom')}
                                style={{
                                    whiteSpace: 'nowrap',
                                    padding: '8px 12px',
                                    borderRadius: '8px',
                ...
                <div className="person-card bride">
                    ...
                    <div className="form-group">
                        ...
                        <div className="input-with-icon">
                            <i className="fas fa-user"></i>
                            <input className="form-input" name="bride_name" value={formData.bride_name} onChange={handlePersonNameChange} placeholder="Enter bride's full name" />
                        </div>
                    </div>
                    <ImageUpload
                        label="Bride's Photo"
                    ...
                    <div className="form-group">
                        ...
                        <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                            <textarea className="form-textarea" name="bride_description" value={formData.bride_description} onChange={handleChange} placeholder="Share her story, personality, and interests..." style={{ flex: 1 }} />
                            <button
                                type="button"
                                onClick={() => fillGeneratedBio('bride')}
                                style={{
                                    whiteSpace: 'nowrap',
                                    padding: '8px 12px',
                                    borderRadius: '8px',
